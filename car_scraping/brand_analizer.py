...
def analise_brands_with_best_price(cars_with_best_price: list):
    ...
    for car in cars_with_best_price:
        print(f'    {car.full_name}, {car.year}, {car.mileage} km, {car.fuel_type}, {car.gearbox},'
              f' {car.status} - {car.price_pln} PLN')
        print(f'        {car.link}')

    print(f'\nTop 5 cars with best price and best mileage:')
    ...
    print(f'\nTop 5 cars with best price and best year:')
...
def get_brands_with_best_price(brand: str, model=list[''], min_year=1885, max_year=2050,
                               ...
                               fuel_type=list[''], gearbox=list[''], status='') -> list:
    """
    ...
    print(f'Average price for {brand} {model} with chosen properties is {average_price}')
